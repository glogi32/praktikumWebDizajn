...
isset($_POST["processor"]) && isset($_POST["camera"]) && isset($_POST["ram"])
  && isset($_POST["ddlType"]) && isset($_POST["ddlBrands"])){
    ...
    if(count($errors) == 0){
        ...
        $uspesanUpload = move_uploaded_file($slikaTmp,$novaPutanja);
        
        if($uspesanUpload){
            $rezultat = $konekcija->prepare("INSERT INTO proizvodi VALUES(NULL,?,?,?,?,?,?,?,?)");
            try{
            ...
            $rezultat = $konekcija->prepare("INSERT INTO slike VALUES(NULL,?,?,?,?)");
